...
CudaOperator::cuAdd(double* src1, double src*2, double* dst, size_t size) {
    std::cout<<"cuAdd()\n";
...
CudaOperator::cuMul(double* A, double* B, double* C) {
...
CudaOperator::cuDot(double* A, double* B, double* C) {
    // dim3 threadsPerBlock(LBLK, LBLK);
    // dim3 blocks(updiv(N, LBLK), updiv(N, LBLK));
    // cudaSimpleKernel<<<blocks, threadsPerBlock>>>(N, dmatA, dmatB, dmatC);
    std::cout<<"cuDot\n";
